...
@section('content')
    ...
    <form method="GET" action="{{ route('dashboard.products.index') }}" class="form-inline">
        並び替え
        <select name="sort" class="form-inline mx-2">
    ...
    <div class="w-75 mt-2">
        <div class="w-75">
            <form method="GET" action="{{ route('dashboard.products.index') }}">
                <div class="d-flex flex-row form-group my-2">
                ...
                <tbody>
                    @foreach( $products as $product)
                        <tr>
                            ...
                            <td>
                                @if($product->image!== "")
                                    <img src="{{ asset('storage/products/' . $product->image) }}" class="w-75 img-fluid">
                                @else
                                    <img src="{{ asset('img/dummy.png') }}" class="w-75 img-fluid">
                                @endif
                            </td>
                            <td>{{ $product->name }}</td>
                            <td>{{ $product->price }}</td>
